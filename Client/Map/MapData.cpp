					...
					if (m_pData[dX][dY].m_cOwnerAction == OBJECTSTOP) {
						...
						case 6: // glowing armor/weapon
							if ((m_pData[dX][dY].m_cOwnerFrame == 1) || (m_pData[dX][dY].m_cOwnerFrame == 5))
							{
								...
								iAngV = m_pGame->CheckAngelicStatus();									// Get Angel value
								if ((((m_pData[dX][dY].m_iStatus & 0x0000F000) >> 8) > rand() % 6)		// Angel stars
									&& ((m_pData[dX][dY].m_iStatus & 0x0000F000) != 0)					// check if angel is present
									&& ((m_pData[dX][dY].m_iStatus & 0x10) == 0) && iAngV > 0)			// check if status invi is on 
								{
									short i;
						...
						case 54: // Dark-Elf
							if ((m_pData[dX][dY].m_cOwnerFrame == 1) || (m_pData[dX][dY].m_cOwnerFrame == 5)) {
									...
									&& ((m_pData[dX][dY].m_iStatus & 0x10) == 0) && iAngV > 0)	// check if status invi is on 
								{
									...
									{
										for (i = 1; i < iAngV; i++) {
											m_pGame->bAddNewEffect(54, (m_sPivotX + dX) * 32 + dx + (rand() % 15 - 23), (m_sPivotY + dY) * 32 + dy - (rand() % 30) - 40, NULL, NULL, -(rand() % 8), 0);
										}
									...
									{
										for (i = 1; i < iAngV; i++) {
											m_pGame->bAddNewEffect(54, (m_sPivotX + dX) * 32 + dx + (rand() % 15 + 10), (m_sPivotY + dY) * 32 + dy - (rand() % 30) - 40, NULL, NULL, -(rand() % 8), 0);
										}
									}
								}
							}
