...
Unit *GetUnit(int handler, int type) {
	if(type == OWNERTYPE_PLAYER) {
		return g_clientList[handler];
	}
	if(type == OWNERTYPE_NPC) {
		return g_npcList[handler];
	}
	return 0;
}
Unit::Unit(UnitType t):m_unittype(t) {
	m_bIsKilled   = FALSE;
	m_killerh = -1;
...
void Unit::RemoveInvisibility() {
	...
	RemoveFromDelayEventList(this, MAGICTYPE_INVISIBILITY);
	m_cMagicEffectStatus[ MAGICTYPE_INVISIBILITY ] = NULL;
}
...
void Unit::SetMagicFlag(short magicType, bool enabled)
{
	...
	switch(magicType)
	{
		...
		case MAGICTYPE_BERSERK:
			flag = STATUS_BERSERK;
			break;
		case MAGICTYPE_RAGE:		// Revan Rage 16 / 05 / 2016
			flag = STATUS_RAGE;
			break;
...
bool Unit::AddMagicEffect(short magicType, long effectTime, char kind)
{
	...
	RegisterDelayEvent(DELAYEVENTTYPE_MAGICRELEASE, magicType, dwTime + (effectTime)_s, 
...
bool Unit::RemoveMagicEffect(char magicType)
{
	...
	RemoveFromDelayEventList(this, magicType);
	return TRUE;
}
