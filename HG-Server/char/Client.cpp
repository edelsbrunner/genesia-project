...
CClient::CClient(HWND hWnd, int clientH): Unit(UNITTYPE_PLAYER)
{
	register int i;
	...
	_angelStr = _angelMag = _angelInt = _angelDex = _angelVit = 0;
}
...
int CClient::GetMaxHP() const
{
	...
	iRet = GetVit()*3 + GetLevel()*2 + GetStr()/2;
...
int CClient::GetFrostResistRatio() {
	...
	// Items that immunizes to frost
	if (m_sItemEquipmentStatus[ EQUIPPOS_BODY ] == 770 || m_sItemEquipmentStatus[ EQUIPPOS_BODY ] == 771 ) {
			return 100;
	}
...
void CClient::WeaponSkillUp(short sWeaponIndex, int iValue) {
	...
	if (!m_pItemList[sWeaponIndex]) return;
	if (IsDead()) return;

	skillIndexes sSkillIndex = (skillIndexes) m_pItemList[sWeaponIndex]->m_sRelatedSkill;
